...
import 'package:stattrack/utils/open_AI.dart';

class RecommendMealForm extends ConsumerStatefulWidget {
  const RecommendMealForm({Key? key, required this.storedIngredients})
      : super(key: key);

  final List<Ingredient> storedIngredients;

  @override
  _RecommendMealFormState createState() => _RecommendMealFormState();
}

class _RecommendMealFormState extends ConsumerState<RecommendMealForm> {
  final TextEditingController _nameController = TextEditingController();
  final TextEditingController _recommendedMealController =
  ...
  /// Gets all of the InstructionFields and returns them as a list of strings

  List<String> _getInstructions() {
    List<String> instructions = [];
    _instructions.forEach((key, value) {
      if (value != null) {
  ...
  List<Ingredient> _getSelectedIngredients() {
    ...
    _ingredients.forEach((key, value) {
      if (value != null) {
        ingredients.add(value.ingredient);
      }
    });
    return ingredients;
  }

  /// Adds a recommended meal to a TextField

  Future<TextField> _recommendMeal() async {
    final List<Ingredient> ingredients = _getSelectedIngredients();
    final String meal =
        await OpenAI.recommendMeal(ingredients, _getInstructions());
    _recommendedMealController.text = meal;

    return TextField(
      controller: _recommendedMealController,
